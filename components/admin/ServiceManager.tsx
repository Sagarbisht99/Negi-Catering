...
export default function ServiceManager() {
  ...
  const save = useMutation({
    mutationFn: async (formData: FormData) => {
      if (editing) await updateService(editing.id, formData);
  ...
  return (
    <section className="space-y-5">
      ...
      <div className={`${adminPanel} overflow-hidden`}>
        ...
        {services.isPending ? <p className="px-5 pb-5 text-sm text-zinc-500">Loading...</p> : null}
        {services.error ? (
          <p className="px-5 pb-5 text-sm font-semibold text-terracotta">
            {(services.error as Error).message}
          </p>
        ) : null}
        {!services.isPending && !rows.length ? (
          <p className="px-5 pb-5 text-sm text-zinc-500">No services yet. Add the first service.</p>
        ) : null}
